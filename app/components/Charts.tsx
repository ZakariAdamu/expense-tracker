...
export default function Charts() {
  ...
  const overviewData: ChartPoint[] = useMemo(
    () => [
      { name: "Income", value: summary.income, fill: COLORS[0] },
      { name: "Spent", value: summary.expenses, fill: COLORS[1] },
      { name: "Savings", value: summary.savings, fill: COLORS[2] },
    ],
    [summary.income, summary.expenses, summary.savings],
  );

  const tickStyle = useMemo(() => ({ fill: "#6b7280" }), []);
  const yTickFormatter = useCallback((value: number) => `$${value}`, []);
  ...
  const renderPieShape = useCallback(
    (props: PieSectorShapeProps, index: number) => (
      <Sector
        {...props}
        fill={props.payload?.fill ?? COLORS[index % COLORS.length]}
      />
    ),
    [],
  );
    ...
    <div className="space-y-6">
      {toast && (
        <div className="fixed right-5 top-5 z-60">
          <div
            className={`rounded-lg px-4 py-3 text-sm shadow-lg border ${toast.type === "success" ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200"}`}
          >
            {toast.message}
          </div>
        </div>
          ...
          {mounted && (
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <YAxis tickFormatter={yTickFormatter} tick={tickStyle} />
                <Tooltip />
                <Legend />
                <Bar
                  dataKey="value"
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-3">
            ...
            {recentExpense.length > 4 && (
              ...
              >
                {showAllExpense ? "Show less" : "Show all"}
              </button>
            )}
          </div>
        </div>
...
function TransactionRow({
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-gray-100 bg-white px-4 py-3 shadow-sm">
      ...
      <p
        className={`font-semibold ${transaction.type === "income" ? "text-teal-600" : "text-orange-600"}`}
      >
