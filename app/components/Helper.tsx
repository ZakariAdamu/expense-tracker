import { TimeFrame } from "../context/OutletContext";
import { NewTransaction } from "./AddTransactions";

// Helper functions for data filtering, calculations, and chart point generation
export const getTimeFrameRange = (timeFrame: TimeFrame) => {
	const now = new Date();
...
export const getPreviousTimeFrameRange = (timeFrame: TimeFrame) => {
	const now = new Date();
	const start = new Date(now);
	start.setHours(0, 0, 0, 0);
...
export const calculateData = (transactions: NewTransaction[]) => {
...
export const generateChartPoints = (timeFrame: TimeFrame) => {
	const now = new Date();
	const points = [];
